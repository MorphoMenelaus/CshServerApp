...
const getMovieData = async (req, res) => {
	...
	try {
		...
		const query = `SELECT ${columns} FROM metadata_items ORDER BY importIndex ASC LIMIT ? OFFSET ?`;
		const rows = await conn.execute(query, [Number(resultLimit), Number(resultOffset)]);
...
module.exports = { getUserLogs, addUserLogs, getClockLog, logSimpleClock, getMovieData };